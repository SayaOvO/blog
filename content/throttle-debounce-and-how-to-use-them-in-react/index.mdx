...
published: true
---
在前端的世界里，会出现频繁的调用事件处理函数的情况，这可能既不必要也会造成性能问题，所以我们需要一些技术来减少这些函数被调用的次数。防抖（debounce）和节流（throttle）就是其中最常用的两个工具函数。

## 防抖和节流的使用场景

当你实现一个搜索功能时，最直接的办法是监听当前搜索框的值，然后使用该值向后端发送请求。举个例子来说，如果你想要搜索 `React`，依照这个逻辑，会依次以 _R_, _Re_, _Rea_, _Reac_, _React_ 为值向后端 API 进行请求。如果按照正常用户一分钟 60 个字来估计，意味着一秒钟就会发送 5/6 个请求，这不太合理，也会降低网页的性能，影响用户体验。在这种情况下，一个简单的解决方式是，等一会直到用户停止输入再进行请求。这就是防抖（debounce)
...
- 延后渲染某些昂贵的组件，使其不要阻塞其他组件的渲染，造成卡顿
```ts
function App() {
...
这个钩子函数和上面其他的应用特别是 sukka 的 useDebouncedValue 区别之处也在 foxact 的文档有说明
