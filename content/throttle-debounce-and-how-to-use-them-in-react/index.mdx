...
在前端的世界里，会出现频繁的调用事件处理函数的情况，这可能既不必要同时也会造成性能问题，所以我们需要一些技术来减少这些函数被调用的次数。防抖（debounce）和节流（throttle）就是其中最常用的两个工具函数。
...
直接控制输出的值，而巧妙之处在于通过 `useEffect` 返回的清理函数来重置定时器，如果传入的 `value` 值更改，在 `useEffect` 的回调函数再一次执行之前会先运行清理函数。